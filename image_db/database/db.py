...
class Db:
    ...
    def __createBlobConnection():
        if not Db.__BlobConnection:
            Db.__BlobConnection = BlobServiceClient.from_connection_string(
                conn_str=Db.__BlobConnectionString
            )
    ...
    def __createContainerConnections():
        ...
        try:
            Db.__ThumbnailConnection = Db.__BlobConnection.get_container_client(
                container=Db.__BlobThumbnailContainer
            )
            Db.__ThumbnailConnection.get_container_properties()
        except:
            Db.__ThumbnailConnection = Db.__BlobConnection.create_container(
                Db.__BlobThumbnailContainer
            )

        try:
            Db.__ImageConnection = Db.__BlobConnection.get_container_client(
                container=Db.__BlobImageContainer
            )
            Db.__ImageConnection.get_container_properties()
        except:
            Db.__ImageConnection = Db.__BlobConnection.create_container(
                Db.__BlobImageContainer
            )
